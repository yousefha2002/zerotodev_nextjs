...
export async function getQuestions(page = 1, limit = 5) {
    return apiGet<{ questions: QuestionList[]; totalPages: number }>(
        `${process.env.API}question/all?page=${page}&limit=${limit}`
    );
}

export async function getLatestQuestions(limit = 5) {
    return apiGet<ContentList[]>(`${process.env.API}question/latest?limit=${limit}`);
}

export async function getRandomQuestions(limit = 5) {
    return apiGet<ContentList[]>(`${process.env.API}question/random?limit=${limit}`);
}
