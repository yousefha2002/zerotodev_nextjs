...
export async function getQuizes(page = 1, limit = 5, name?: string) {
  const nameQuery = name ? `&name=${name}` : "";

  return apiGet<{ quizes: QuizSummary[]; totalPages: number }>(
    `${process.env.API}quiz/all?page=${page}&limit=${limit}${nameQuery}`
  );
}
...
export async function getViewQuiz(id: number) {
  ...
  }>(`${process.env.API}quiz/user/${id}`, tokenValue);
}

export async function getQuizWithQuestions(id: number) {
  const token = await getUserToken();
  const tokenValue = token ? token.value : undefined;
  return apiGetOrNull<QuizWithQuestions>(
    `${process.env.API}quiz/user/${id}/questions`,
    tokenValue
  );
}
